...
export class ResponseNode {
  ...
  private generateFallbackResponse(state: AgentState): string {
    ...
    if (message.includes('salut') || message.includes('bună') || message.includes('hello')) {
      switch (businessType) {
        case 'dental': return 'Salut! Sunt asistentul virtual al cabinetului dental. Cum vă pot ajuta astăzi?';
        case 'gym': return 'Salut! Sunt asistentul virtual al salii de fitness. Cum vă pot ajuta astăzi?';
        case 'hotel': return 'Salut! Sunt asistentul virtual al hotelului. Cum vă pot ajuta astăzi?';
        default: return 'Salut! Sunt asistentul virtual al business-ului. Cum vă pot ajuta astăzi?';
      }
    }
