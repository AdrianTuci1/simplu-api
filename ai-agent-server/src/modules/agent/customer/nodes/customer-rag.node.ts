...
export class CustomerRagNode {
  ...
  async invoke(state: AgentState): Promise<Partial<AgentState>> {
    try {
      // Get customer-specific RAG instructions
      const ragInstructions = await this.ragService.getCustomerInstructions(
      ...
      console.warn('CustomerRagNode: error loading RAG instructions', error);
      return {
        ragResults: [],
        systemInstructions: []
      };
  ...
  private async filterRelevantInstructions(
    ...
    const prompt = `
    Ești un agent AI pentru clienți. Analizează instrucțiunile RAG și determină care sunt relevante pentru cererea curentă.
    
    Context:
    - Business: ${state.businessInfo?.businessName || 'necunoscut'} (${state.businessInfo?.businessType || 'general'})
    - Mesaj: "${state.message}"
    - Rol: customer
    - Capabilități: service_discovery, booking_guidance, treatment_information, customer_support
